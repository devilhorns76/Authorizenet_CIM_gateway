<?php		
	global $gateway, $pmpro_review, $skip_account_fields, $pmpro_paypal_token, $wpdb, $current_user, $pmpro_msg, $pmpro_msgt, $pmpro_requirebilling, $pmpro_level, $pmpro_levels, $ifi_addon_level, $tospage, $pmpro_show_discount_code, $pmpro_error_fields;
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<input type="hidden" id="billing_required" name="billing_required" value="" />
	
	<?php
		if(isset($_SESSION['CIM_profile_id']))
			{ ?>
			<input type="hidden" id="CIM_profile_id" name="CIM_profile_id" value="<?php echo $_SESSION['CIM_profile_id'] ?>" />
			<?php unset($_SESSION['CIM_profile_id']);
			} ?>
	<?php
		if(isset($_SESSION['payment_profile_id'])) 
			unset($_SESSION['payment_profile_id']);
			
		if(isset($_SESSION["create_payment_profile_error"]))
			{ 
			$pmpro_msg = $_SESSION["create_payment_profile_error"]; 
			unset($_SESSION["create_payment_profile_error"]);
			}
	?>
	...
	<thead>
		<tr>
			<th>
				<?php if(count($pmpro_levels) > 1) { ?><?php } ?><?php _e('Secure Checkout', 'pmpro');?>
			</th>						
		</tr>
	</thead>
	<tbody>                
	...
	<?php if(!$skip_account_fields && !$pmpro_review) { ?> <!-- account setup section - username, password, etc -->
	<div  style="max-width:615px; margin-left:auto; margin-right:auto;">
	...
	<?php } elseif($current_user->ID && !$pmpro_review) { ?>    <!-- if user is already a member and logged in -->                    	                       										
		
		<p id="pmpro_account_loggedin" style="text-align:center; font-size:.9em;">
			<?php printf(__('You are logged in as <strong>%s</strong>.<a href="%s">Not you?</a>.', 'pmpro'), $current_user->user_login, wp_logout_url($_SERVER['REQUEST_URI'])); ?>			
		</p>
	<?php } ?>
	...
	?>		
		...
		<!-- if user is already customer and logged in AND CIM gateway -->
	<?php if(pmpro_getOption("gateway", true) == "authorizenet_CIM" && $current_user->ID) { ?>
		<div  style="max-width:615px; margin-left:auto; margin-right:auto;">
		...
		<tbody>
			<tr>
				<td>
					<div class="ifi_payment_profiles">
						<?php $CIM_profile_id = get_user_meta($current_user->ID, "CIM_profile_id", true); // returns user CIM_profile_id ?>
						<input type="hidden" name="CIM_profile_id" value="<?php echo $CIM_profile_id ?>">
							<?php
							// look up current_user's available payment profile ids
							$ifi_payment_profile_ids = get_user_meta($current_user->ID, "ifi_payment_profile_ids", true); // returns all user payment profiles as array 
							
							// count number of payment profiles that are in this array
							$number_of_ids = count($ifi_payment_profile_ids);
							//var_dump($number_of_ids);
							
							$path = "/images/";
							$scheme = 'https';
							$site_url = site_url($path, $scheme);
							$images_path = plugins_url( 'images/' , __DIR__);
							foreach($ifi_payment_profile_ids as $ifi_payment_profile_id) // gives me each payment_profile_id individually
								{
										foreach($ifi_payment_profile_id as $key=>$value)
											{
												$$key = $value; // gives me each piece of the payment_profile_id by name
											}
												?>
												<div class="pmpro_payment-profile">
													<input type="radio" name="payment_profile_id" value="<?php echo $payment_profile_id ?>" />
														<img class="ifi_card_sm" src="<?php echo $images_path . $cardtype . '_sm.png' ?>" width="80px" height="50px" alt="<?php echo $cardtype ?>">&nbsp;&nbsp;
														<?php echo "ending in " . trim($accountnumber, "X-") ?>&nbsp;&#8212;&nbsp;expires:&nbsp;<?php echo $expiration_month . " /" ?>&nbsp;<?php echo $expiration_year ?>
												</div>
												<?php
											//}
										//}
								}
							// include option for adding a new payment method
							?>
								<div class="pmpro_payment-profile_new">
									<input type="radio" style="vertical-align:top;"name="payment_profile_id" id="add_new_payment_profile" value="" />&nbsp;&nbsp;Add a New Payment Method
									
									<!-- script to toggle billing and payment info fields display -->
									<script type ="text/javascript">
									$('.add_new').hide();
									 $('input[type=radio]').click(function(){
										 if($('input[type=radio]:checked') && $(this).attr('value')==0){
											 if(!($('#pmpro_billing_address_fields').is(':visible'))){
												 $('#pmpro_billing_address_fields').slideToggle('fast')
												 $('#pmpro_payment_information_fields').slideToggle('fast')
												 //$('#billing_required').val("yes");
											 }
										 }else if($('input[type=radio]:checked') && $(this).attr('value')>1){
											 $('#pmpro_billing_address_fields') .fadeOut("fast");
											 $('#pmpro_payment_information_fields') .fadeOut("fast");
											 $('#AccountNumber').val("0000000000000000");
											 $('#CardType').val("CIM Payment Profile");
											 $('#CVV').val("000");
										 }
										else if(!($('#pmpro_billing_address_fields').is(':visible'))){
										   $('#pmpro_billing_address_fields').slideToggle('fast');
										   $('#pmpro_payment_information_fields').slideToggle('fast');
										   //$('#billing_required').val("yes");
										 }
									...
									<!-- end script to toggle billing and payment info fields display -->
								</div>
					</div>
				</td>
			</tr>
	...
	<?php 
		...
		{
	?>
	<div align="center"  style="max-width:615px; margin-left:auto; margin-right:auto;">
	<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_billing_address_fields", false)  || $CIM_profile_id ){ ?>style="display: none;"<?php } ?>>
	<thead>
		<tr>
	...
	<tbody>
		<tr valign="top" style="display:block !important; margin:0 auto !important; width:615px !important;">
			<td style="position:relative;">
				...
				?>
				<div>
					<label for="bemail"><?php _e('E-mail', 'pmpro');?></label>
					<input id="bemail" name="bemail" type="text" class="input <?php echo pmpro_getClassForField("bemail");?>" size="30" value="<?php echo esc_attr($bemail)?>" /> 
				</div>
				<?php
					$pmpro_checkout_confirm_email = apply_filters("pmpro_checkout_confirm_email", true);					
	...
	<?php		
		...
		?>
		<div align="center"  style="max-width:615px; margin-left:auto; margin-right:auto;">
		<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if(!$pmpro_requirebilling || apply_filters("pmpro_hide_payment_information_fields", false) || $CIM_profile_id ) { ?>style="display: none;"<?php } ?>>
		<thead>
			<tr>
				<th><?php _e('Payment Information', 'pmpro');?></th>
			</tr>
		</thead>
		<tbody style="display:block !important; margin-left: auto !important; margin-right:auto !important; width:615px !important;">                    
			<tr valign="top">		
				<td style="position:relative;">	
				<span class="pmpro_thead-msg"><?php printf(__('We Accept %s', 'pmpro'), $pmpro_accepted_credit_cards_string);?></span>
					...
					<div class="pmpro_payment-expiration">
						...
						<select id="ExpirationMonth" name="ExpirationMonth" class=" <?php echo pmpro_getClassForField("ExpirationMonth");?>" style="width:72px !important;">
						...
						</select>&nbsp;/&nbsp;<select id="ExpirationYear" name="ExpirationYear" class=" <?php echo pmpro_getClassForField("ExpirationYear");?>" style="width:90px !important;">
							<?php
					...
					<div class="pmpro_payment-cvv">
						...
						<input class="input" style="width:72px !important;" id="CVV" name="CVV" type="text" size="4" value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class=" <?php echo pmpro_getClassForField("CVV");?>" />  <small style="font-size:11pt;">(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter(PMPRO_URL)?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _ex("what's this?", 'link to CVV help', 'pmpro');?></a>)</small>
					</div>
					<?php
						}
					?>
					...
					<div class="pmpro_payment-discount-code">
						...
						<input class="input <?php echo pmpro_getClassForField("discount_code");?>" id="discount_code" name="discount_code" type="text" style="width:220px !important;"size="20" value="<?php echo esc_attr($discount_code)?>" />
...
<?php echo wpautop($tospage->post_content)?>
	...
	<div class="pmpro_submit" style="position:relative;">
					<?php
