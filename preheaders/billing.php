...
// start my edits
	$passedlid = $_REQUEST['lid'];
	$ifisublevel_info = $wpdb->get_row("SELECT * FROM $wpdb->pmpro_membership_levels WHERE id = $passedlid",ARRAY_A); // returns level info as array
			foreach($ifisublevel_info as $key=>$value)
			{ 
				$$key = $value; // now the passedlid level's info can be called as variables - $id, $name, $billing_amount, etc
			}
			$ifi_invoices = get_user_meta($current_user->ID, "ifi_customer_invoices", true);
			$ifi_sub_invoice_info = search($ifi_invoices, 'lid', $passedlid);
			foreach($ifi_sub_invoice_info as $ifi_sub)
			foreach($ifi_sub as $key=>$value){
				$$key = $value;} // gives me $invoice_id, $lid, $sub_id


//$ifi_sub_id = IFIgetLastMemberSubOrder();
//var_dump($ifi_sub_id);
//die;
//$lid = $_REQUEST['lid'];
	//		$ifi_invoices = get_user_meta($current_user->ID, "ifi_customer_invoices", true);
		//	$ifi_sub_invoice_info = search($ifi_invoices, 'lid', $lid);
			//foreach($ifi_sub_invoice_info as $key=>$value){
				//$$key = $value;} // gives me $invoice_id
...
//$id = $sub_id;
    $ifi_sub_order_id = $wpdb->get_var("SELECT id FROM $wpdb->pmpro_membership_orders WHERE subscription_transaction_id = '" . $sub_id . "'");
$id = $ifi_sub_order_id;
//$user_order = new MemberOrder();
//$user_order->getMemberOrderByID($id);
//$user_order->id = $ifi_sub_order_id;

//var_dump($user_order);
//echo "=== user_order above ====";
//var_dump($sub_id);
//echo "=== sub_id above ====";
//var_dump($lid);
//echo "=== lid above ====";
//var_dump($invoice_id);
//echo "=== invoice_id above ====";
...
if ($submit) 
{
	...
	else 
	{
        ...
        //change this
        //$order_id = $wpdb->get_var("SELECT id FROM $wpdb->pmpro_membership_orders WHERE user_id = '" . $current_user->ID . "' AND membership_id = '" . $current_user->membership_level->ID . "' AND status = 'success' AND payment_transaction_id = '" . $invoice_id . "'ORDER BY id DESC LIMIT 1");
        //$order_id = $wpdb->get_var("SELECT subscription_transaction_id FROM $wpdb->pmpro_membership_orders WHERE payment_transaction_id = '" . $invoice_id . "'");
		//$invoice_id = $_REQUEST['ifi_sub_id'];
	    //$order_id = $wpdb->get_var("SELECT id FROM $wpdb->pmpro_membership_orders WHERE ifi_sub_id = '" . $invoice_id . "'");
		$order_id = $_REQUEST['ifi_sub_order_id'];
		...
		//$morder->subscription_transaction_id = $order_id;
		//var_dump($morder);
			...
			//$worked = pmprogateway_authorizenet_CIM::update($morder);
//echo "update billing preheader worked info below ===" . "<br>";
//var_dump($worked);
//var_dump($morder->status);
//var_dump($morder->error);
//die;
            if ($worked) 
			{
...
else //load up default values for them
{
