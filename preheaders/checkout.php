<?php
	...
	// add in CIM stuff
		//check to see if there's a CIM_profile_id in the session because card was invalid on 1st attempt
	if(isset($_SESSION["CIM_profile_id"]))
		$CIM_profile_id = $_SESSION["CIM_profile_id"];
		// if there's no existing morder, get the hidden field
	elseif(isset($_REQUEST['CIM_profile_id']))
...
//var_dump($pmpro_required_billing_fields);
//var_dump($pmpro_required_user_fields);
	...
	if ($submit && $pmpro_msgt != "pmpro_error")	{
		...
		if ($pmpro_requirebilling && empty($CIM_profile_id) && empty($payment_profile_id)) {		
			//filter							
		...
		//if(!empty($CIM_profile_id) && !empty($payment_profile_id) && empty($AccountNumber))
		//	{
		//		$pmpro_continue_registration = true;
		//	}
	...
	if (empty($submit)) {
		...
		if (!empty($current_user->ID)) {
			...
			$AccountNumber = get_user_meta($current_user->ID, "pmpro_AccountNumber", true);
			if(!isset($CVV)) 
				$CVV = "000";
		}	
	}
